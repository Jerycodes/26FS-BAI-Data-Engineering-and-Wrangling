...
for pair in ["EUR_USD", "GBP_USD"]:
    ...
    ax1.plot(pw.index, pw.values, color="#333333", lw=1.6, label="Kursniveau (Wochenschluss)")
    ax1.set_ylabel("Kursniveau", color="#333333")
    ax1.tick_params(axis="y", labelcolor="#333333")
    ax2 = ax1.twinx()
    ax2.axhline(0, color="grey", ls=":", lw=0.8)
    ax2.bar(sw.index, sw.values, width=5, color=np.where(sw.values >= 0, "#0072B2", "#D55E00"), alpha=0.6)
    ax2.set_ylabel("Wochen-Sentiment (blau positiv, orange negativ)", color="#555555")
    ax2.grid(False)
...
ax.bar(x - 0.2, before, 0.4, label="vor Ausrichtung", color="#E69F00")
ax.bar(x + 0.2, after, 0.4, label="nach Ausrichtung", color="#0072B2")
ax.set_xticks(x)
...
ax.plot([-1, 1], [-1, 1], color="#555555", lw=1, ls="--", label="vollstaendige Uebereinstimmung")
